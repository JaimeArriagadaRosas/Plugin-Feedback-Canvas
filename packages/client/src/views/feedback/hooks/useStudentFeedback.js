...
export function useStudentFeedback(initialStudentId = 1) {
  ...
  const { data: assignments = [], isLoading: loading } = useQuery({
    ...
    queryFn: async () => {
      const result = await api.get(`/student/feedback/${studentId}`);
  ...
  const rateMutation = useMutation({
    mutationFn: async ({ feedbackId, rating }) => {
      const result = await api.post('/student/rate', { feedbackId, rating });
