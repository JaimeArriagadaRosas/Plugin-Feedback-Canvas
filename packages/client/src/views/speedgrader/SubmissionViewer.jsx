...
export default function SubmissionViewer({
  ...
  useEffect(() => {
    if (hasPreviewUrl && !isFile && !hasBody && !iframeLoaded && !iframeTimedOut) {
      timeoutRef.current = setTimeout(() => {
        console.warn(`[SubmissionViewer] Iframe timeout (${IFRAME_TIMEOUT_MS}ms) — preview_url no respondió.`);
  ...
  if (submission) {
    const branch = !submission ? 'NO_SUBMISSION'
      ...
      : 'FALLBACK_EMPTY';
    console.log(`[SubmissionViewer] Rama: ${branch} | type=${submission.submission_type || 'N/A'} | attachments=${submission.attachments?.length || 0} | hasBody=${hasBody} | hasPreviewUrl=${hasPreviewUrl}`);
  }
  ...
  const renderContent = () => {
    ...
    if (isFile && fileUrl) {
      ...
      const proxyUrl = `/api/file/preview?url=${encodeURIComponent(fileUrl)}`;
      return <NativePdfViewer fileUrl={proxyUrl} />;
    }

    // 3. PRIORIDAD: Contenido de texto/HTML de la entrega (online_text_entry)
    //    Se evalúa ANTES del iframe de preview_url porque Canvadocs no funciona en local
    if (hasBody) {
      return (
        <div className={styles.paper}>
          <hr className={styles.divider} />
    ...
    return (
      <div className={styles.paper}>
        <hr className={styles.divider} />
        <p className={styles.text}>{textBody || "Sin contenido de entrega."}</p>
      </div>
    );
  };
