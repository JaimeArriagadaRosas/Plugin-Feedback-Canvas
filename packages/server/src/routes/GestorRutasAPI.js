...
export default class GestorRutasAPI {
  ...
  configurarRutasProtegidas() {
    ...
    this._configurarRutasCursos(canvasOAuth);
    this._configurarRutasPlantillas();
    this._configurarRutasFeedback();
    this._configurarRutasEstadisticas();
    this._configurarRutasWebhooks();
    this._configurarRutasEstudiantes();
    this._configurarRutasConfiguracionAdmin();

    logger.debug('Rutas protegidas de la API configuradas correctamente');
  }

  _configurarRutasCursos(canvasOAuth) {
    this.router.get('/courses', authorizeRole(['teacher']), canvasOAuth, handleValidationErrors, (req, res, next) => { logger.debug('GET /courses', { user: req.ltiContext?.user }); this.courseCtrl.getCourses(req, res, next); });
  ...
  _configurarRutasEstadisticas() {
    this.router.get('/stats/course/:courseId', authorizeRole(['teacher', 'admin']), ...validateCourseId, handleValidationErrors, (req, res, next) => this.statsCtrl.getCourseStats(req, res, next));
    this.router.get('/stats/grades/:courseId', authorizeRole(['teacher', 'admin']), ...validateCourseId, handleValidationErrors, (req, res, next) => this.statsCtrl.getGradeDistribution(req, res, next));
    this.router.get('/stats/ratings/:courseId', authorizeRole(['teacher', 'admin']), ...validateCourseId, handleValidationErrors, (req, res, next) => this.statsCtrl.getStudentRatings(req, res, next));
    this.router.get('/stats/export/:courseId', authorizeRole(['teacher', 'admin']), ...validateCourseId, handleValidationErrors, (req, res, next) => this.statsCtrl.exportCsv(req, res, next));
    this.router.get('/audit/logs', authorizeRole(['admin']), (req, res, next) => this.auditLogCtrl.getLogs(req, res, next));
  }

  _configurarRutasWebhooks() {
    this.router.post('/webhooks/canvas', webhookLimiter, (req, res, next) => this.webhookCtrl.handleWebhook(req, res, next));
  }

  _configurarRutasEstudiantes() {
    this.router.get('/student/feedback/:studentId', authorizeRole(['student', 'teacher']), ...validateStudentId, requireDeploymentId, handleValidationErrors, (req, res, next) => this.feedbackCtrl.getStudentView(req, res, next));
    this.router.post('/student/rate', authorizeRole(['student']), requireDeploymentId, validateBody(schemas.studentRate), (req, res, next) => this.feedbackCtrl.rateByStudent(req, res, next));
  }

  _configurarRutasConfiguracionAdmin() {
