...
export class CanvasBringup {
  ...
  async ensureRubyDependencies() {
    ...
    const fixCmd = "gem install bundler-multilock -v 1.1.4"; // Or just install it
    await runCommand('docker', ['compose', 'exec', '-T', 'web', 'bash', '-c', fixCmd], { cwd: this.canvasDir });

    const installCmd = "find gems -maxdepth 2 -name Gemfile.lock -delete 2>/dev/null; bundle config set --local frozen false && bundle install --jobs=2";
  ...
  async waitForReady(timeout = 180, interval = 5) {
    ...
    while (elapsed < timeout) {
      const { success, out } = await runCommand('docker', ['compose', 'ps', '-q', 'web'], { cwd: this.canvasDir });
      if (success && out.trim()) {
