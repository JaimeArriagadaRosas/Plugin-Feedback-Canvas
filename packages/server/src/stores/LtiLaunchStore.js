...
export async function initLtiStore() {
  if (process.env.REDIS_URL && !redisClient) {
    try {
      ...
      console.info('LTI Launch Store inicializado con Redis', { url: process.env.REDIS_URL });
    } catch (err) {
      console.warn('Falló conexión Redis, usando store en memoria', { error: err.message });
      memoryStore = new Map();
    }
  }
}
