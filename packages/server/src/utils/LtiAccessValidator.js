...
export const validateLtiLaunch = (tokenOrDecoded) => {
  try {
    ...
    if (!decoded) {
      console.warn('[LTI] Token recibido nulo o mal formado');
      return false;
    }

    if (decoded.exp && decoded.exp < Math.floor(Date.now() / 1000)) {
      console.warn('[LTI] Token expirado');
      return false;
    }

    const roles = decoded['https://purl.imsglobal.org/spec/lti/claim/roles'];
    if (!roles || !Array.isArray(roles)) {
      console.warn('[LTI] Token sin claim de roles válido');
      return false;
    }

    return isLaunchAllowed(decoded);
  } catch (err) {
    console.error('[LTI] Token inválido:', err.message);
    return false;
  }
}
