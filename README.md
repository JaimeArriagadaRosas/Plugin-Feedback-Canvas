...
2.  **Docker Desktop:** Requerido únicamente para correr la versión local y completa de Canvas LMS.
    ...
    *   **Configuración de memoria:** Antes de usar la Opción 3, asigna al menos **8 GB de RAM** a Docker Desktop (recomendado 10 GB). Ve a **Settings > Resources > Memory** y ajusta el deslizador. Si Docker tiene menos de 8 GB, el preflight mostrará una advertencia y Canvas puede volverse inestable.
    *   **Uso:** Basta con tener la aplicación Docker Desktop abierta ejecutándose en segundo plano antes de iniciar la instalación de Canvas. No requieres realizar ninguna otra configuración interna en Docker.
...
3.  **Creación e Inicialización de Contenedores Docker:**
    ...
    *   **Servidor de Aplicaciones (Contenedor Web):** Encargado de procesar y mostrar la plataforma Canvas LMS en tu navegador (expuesto internamente en el puerto 8080).
...
4.  **Despliegue del Proxy TLS Inverso (Seguridad):**
    El orquestador levanta automáticamente un proxy (`scripts/tls-proxy`) que escucha en el puerto `8443`. Este componente es vital porque envuelve el tráfico local de Canvas LMS en una capa segura HTTPS utilizando certificados locales. Esto garantiza que la integración LTI con el plugin funcione correctamente sin bloqueos de seguridad en tu navegador.
5.  **Instalación de Dependencias de Ruby/Rails:**
...
6.  **Configuración e Inicialización de la Base de Datos:**
...
7.  **Compilación de Assets (Recursos Visuales):**
...
npm run server
```
El servidor backend arrancará en el puerto `https://localhost:3000` (TLS con certificados mkcert locales).

#### Pestaña 2 — Iniciar Frontend de Desarrollo (Vite + React)
```bash
npm run dev
```
El frontend de desarrollo estará disponible en `https://localhost:5173/`.
...
#### Resolución del dominio y puertos de Canvas (canvas.docker)

Canvas LMS en desarrollo usa internamente el dominio `canvas.docker` y su contenedor web publica en el puerto `8080`. Sin embargo, para evitar errores de contenido mixto (Mixed Content) al integrarse con el plugin LTI, el orquestador levanta un **Proxy TLS Inverso** en el puerto `8443` (`https://localhost:8443`). Este proxy redirige de forma segura todo el tráfico HTTPS externo hacia el puerto `8080` del contenedor.

Para que Canvas genere URLs correctas bajo HTTPS y coincida con tu entorno local, mapea `canvas.docker` a `127.0.0.1` en tu archivo de hosts (requiere privilegios de administrador/root):

```bash
npm run setup:hosts          # añade 127.0.0.1 canvas.docker a hosts
# npm run setup:hosts -- --remove   # para revertir
```

Tras esto, para interactuar con Canvas desde tu navegador debes acceder siempre a través de la ruta segura del proxy TLS: `https://localhost:8443`.
