...
describe('Gem Cache Normalization and Retries', () => {
  ...
  it('abort retries immediately if the error is due to insecure permissions', async () => {
  ...
  it('maintains retries for transient errors', async () => {
...
describeBash('Gem Cache Bash Normalization Script', () => {
  ...
  it('b) simulated chmod failed -> exit != 0 and INSECURE_CHMOD_FAILED', () => {
    const insecureDir = path.join(tmpDir, 'insecure');
    fs.mkdirSync(insecureDir);
    fs.chmodSync(insecureDir, 0o777);
