...
/**
 * Module to read and modify the .env file dynamically.
 */
...
export async function ensureEnvConfigured(pluginDir, configData) {
  ...
  try {
    ...
    console.log(pc.blue('ℹ️ Existing .env file found. Its values will be updated.'));
  } catch (e) {
    console.log(pc.blue('ℹ️ .env does not exist. Attempting to use template...'));
    try {
      envContent = await fs.readFile(envExamplePath, 'utf8');
    } catch (err) {
      console.log(pc.yellow('⚠️ .env.example not found. Using native security template.'));
      envContent = `# Feedback Plugin - Autogenerated (Production)\n
CANVAS_BASE_URL=
CANVAS_ISSUER=
CANVAS_ACCESS_TOKEN=
DB_HOST=localhost
...
`;
  ...
  // Generate secrets if they do not exist or force if we are in a new setup
  ...
  const updates = {
    ...
    'USE_LOCAL_DATA': 'false',
    'VITE_USE_LOCAL_DATA': 'false',
    'STARTUP_MODE': '1' // By default we force it to 1 (production) after setup
  };

  // Add developer key and admin token if provided
  ...
  const updatedLines = lines.map(line => {
    ...
    // If encryption keys are empty or in placeholder, fill them
  ...
  // If LTI_CLIENT_ID was not in the base file (e.g. it was commented) but we have updates, append it.
  ...
  console.log(pc.green('✅ .env file successfully configured with generated secrets.'));
}
