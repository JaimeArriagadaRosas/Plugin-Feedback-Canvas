 ...
 * Refactorizado desde el antiguo directorio scripts/ a apps/installer/
 */
...
async function main() {
  ...
  if (httpHits.length === 0) {
    ...
    if (expectedCount > 0) info(`${expectedCount} URL HTTP internas permitidas`, 'Gotenberg, Canvas interno o allowlist local');
    const otherCount = httpHits.length - local.length;
    if (otherCount > 0) info(`Otras ${otherCount} URL http externas`, 'revisar manualmente');
  }
  ...
  if (state.failures === 0) {
    ...
    log(`${C.gray}   Ejecuta: npm run verify:https para corregir las de bajo riesgo.${C.reset}\n`);
  }
