...
describe('FileController Preview Contract', () => {
  ...
  it('Non-existent OAuth Token -> Local validation does not fail, but Canvas API may fail if it requires auth', async () => {
  ...
  it('Gotenberg unreachable (Healthcheck fails due to network)', async () => {
    mockReq.query.url = 'http://canvas.docker/files/123/download?file=doc.docx';
    mockApiSuccess('http://canvas.docker/files/123/download?token=actual');
    mockFetchSuccess(Buffer.from('docx_data'), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'); // file fetch
  ...
  it('Gotenberg returns 5xx during conversion (healthy in healthcheck)', async () => {
    mockReq.query.url = 'http://canvas.docker/files/123/download?file=doc.docx';
    mockApiSuccess('http://canvas.docker/files/123/download?token=actual');
    mockFetchSuccess(Buffer.from('docx_data'), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'); // file fetch
  ...
  it('File larger than 25 MB', async () => {
    mockApiSuccess('http://canvas.docker/files/123/download?token=actual');
  ...
  it('Host/protocol not allowed', async () => {
  ...
  it('Removes Authorization header if downloadUrl is cross-origin', async () => {
  ...
  it('Keeps Authorization header if downloadUrl is same-origin', async () => {
    mockReq.query.url = 'http://canvas.docker/files/123/download?file=doc.pdf';
    mockReq.canvasToken = 'CANVAS_BEARER';
  ...
  it('Secrets redacted upon errors', () => {
