...
export function createConfigRoutes(configCtrl, iaConfigCtrl, permissionsCtrl, variableCtrl) {
  ...
  router.put('/ia-model', authorizeRole(['admin']), validateBody(schemas.iaModel), (req, res, next) => configCtrl.setIAModel(req, res, next));
  router.get('/tokens', authorizeRole(['admin']), (req, res, next) => configCtrl.getTokens(req, res, next));
  router.post('/tokens', authorizeRole(['admin']), validateBody(schemas.iaToken), (req, res, next) => configCtrl.saveToken(req, res, next));
  router.get('/ia-models', authorizeRole(['admin']), (req, res, next) => configCtrl.getAvailableModels(req, res, next));
  
  router.get('/ia-advanced', authorizeRole(['admin', 'teacher']), (req, res, next) => iaConfigCtrl.getConfig(req, res, next));
  router.put('/ia-advanced', authorizeRole(['admin']), validateBody(schemas.iaAdvancedConfig), (req, res, next) => iaConfigCtrl.updateConfig(req, res, next));
