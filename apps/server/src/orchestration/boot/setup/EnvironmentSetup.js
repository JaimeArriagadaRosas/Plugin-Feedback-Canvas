  ...
  async ensureSetup() {
    ...
    if (!allOk) {
      ...
      if (missing.missing_canvas_clone) {
        ...
        if (!(await cloner.cloneCanvas())) throw new Error('Fallo durante el clonado e inicialización base de Canvas LMS (Revise los logs para más detalle)');
      ...
      if (missing.missing_plugin_db) {
        ...
        this.boot.plain('=========================================================');
        this.boot.info('Levantando infraestructura local (Docker)...');
        try {
          await execa('docker', ['compose', '-f', 'docker-compose.db.yml', 'up', '-d', '--wait'], { cwd: this.pluginDir });
          this.boot.info('Ejecutando migraciones (PostgreSQL)...');
      ...
      this.boot.plain('=========================================================');
      const postInstallPreflight = new PreflightChecks(this.boot, this.canvasDir, this.pluginDir);
      const postInstall = await postInstallPreflight.runChecks();
