...
function runNpmInstall() {
  console.warn('\n[!] Instalando/Reparando dependencias (npm install). Esto puede tardar unos minutos...');
  const result = spawnSync('npm', ['install', '--no-fund', '--no-audit', '--loglevel=error'], { cwd: rootDir, stdio: 'inherit', shell: true });
  if (result.error) {
    console.error('\n[X] Falló la ejecución de npm:', result.error.message);
    process.exit(1);
  }
  if (result.status !== 0) {
...
function restartApp() {
  ...
  const result = spawnSync(process.execPath, [__filename, ...args], { cwd: rootDir, stdio: 'inherit' });
