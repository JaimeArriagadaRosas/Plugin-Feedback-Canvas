...
export default function AdminPanel({ onExit }) {
  ...
  const handleSaveTokenConfig = async () => {
    ...
    // Validar longitud mínima de la API key (al menos 10 caracteres para ser razonable)
    if (apiKey.trim().length < 10) {
      ...
      try {
        ...
        const response = await fetch('/api/config/tokens', {
          ...
          body: JSON.stringify({
            servicio: service.toLowerCase(), // OpenAI, Claude, Gemini
            key: apiKey.trim()
          }),
        ...
        {activeTab === "RF55" && (
             ...
             <div style={styles.card}>
               ...
               <div style={styles.row}>
                 <div style={{ flex: "0 0 400px" }}>
                   ...
                   {/* Mensajes de validación y éxito para tokens */}
                   {tokenValidationError && (
                     <div style={{
                       ...
                       ⚠️ Error de Validación: {tokenValidationError}
                     </div>
                   )}
                   ...
                   {tokenSaveSuccess && (
                     <div style={{
                       ...
                       ✅ ¡Token guardado exitosamente! La API key para {service} ha sido actualizada.
                     </div>
                   )}
         ...
         <div style={{ marginTop: "30px", textAlign: "right", display: "flex", justifyContent: "flex-end", gap: "15px" }}>
           <button style={styles.btnSecondary} onClick={() => {
             ...
             // También limpiar estado de token si estamos en esa pestaña
             if (activeTab === "RF56") {
               setApiKey("");
               setTokenValidationError("");
               setTokenSaveSuccess(false);
