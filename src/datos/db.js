...
let pool;
let isLocal = process.env.VITE_USE_LOCAL_DATA === 'true' || !process.env.DB_HOST;
...
if (!isLocal) {
  ...
  pool.on('error', (err) => {
    ...
    console.warn('[DB] No se pudo inicializar el pool de conexiones. Usando modo LOCAL.');
    isLocal = true;
  }
} else {
...
const localDb = {
  query: async (text, params) => {
    ...
    // 6. Simular Configuracion_Curso_Tarea
    if (text.includes('Configuracion_Curso_Tarea')) {
      if (text.includes('SELECT')) {
        const id = params && params[0] ? params[0] : '101';
        return {
          rows: [{
            id: 1,
            contexto_tipo: 'tarea',
            contexto_id: String(id),
            config_json: { pluginActivo: true, plantillaId: 1 }
          }]
        };
      }
      if (text.includes('INSERT') || text.includes('UPDATE')) {
        const configVal = params[2];
        return {
          rows: [{
            id: 1,
            contexto_tipo: params[0],
            contexto_id: String(params[1]),
            config_json: typeof configVal === 'string' ? JSON.parse(configVal) : configVal
          }]
        };
      }
    }

    // 6b. Simular configuracion_asignacion (por compatibilidad si es requerida)
...
export default {
  query: async (text, params) => {
    if (isLocal) return localDb.query(text, params);
    try {
      return await pool.query(text, params);
    } catch (error) {
      const errorMsg = error.errors ? error.errors.map(e => e.message).join(', ') : error.message;
      console.warn(`[DB] Error en base de datos real: ${errorMsg}. Cambiando a LOCAL.`);
      isLocal = true;
