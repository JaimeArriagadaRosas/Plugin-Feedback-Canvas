...
let pool;
let isLocalMode = process.env.VITE_USE_LOCAL_DATA === 'true' || !process.env.DB_HOST;
...
if (!isLocalMode) {
  try {
    ...
    console.warn('[DB] No se pudo inicializar el pool de conexiones. Usando modo LOCAL de respaldo.');
    isLocalMode = true;
  }
} else {
  console.log('[DB] Inicializando base de datos en modo LOCAL.');
}
 ...
 * Retorna respuestas locales de éxito para que el flujo de orquestación funcione.
 */
const localDb = {
  query: async (text, params) => {
    console.log(`[DB-LOCAL] Ejecutando: ${text.substring(0, 100)}...`);
    
    // 1. Procesar búsqueda de plantilla local
    ...
    // 2. Procesar búsqueda de historial académico local (por estudiante)
    ...
    // 3. Procesar INSERT local en historial_feedback
    ...
    // 3b. Procesar SELECT local en Historial_Feedback_Generado
    ...
    // 3c. Procesar UPDATE local en Historial_Feedback_Generado (ratings)
    ...
    // 4. Procesar INSERT local en Historial_Academico_Local
    if (text.includes('INSERT INTO Historial_Academico_Local')) {
      return { rows: [{ id: 1 }] };
    }

    // 6. Procesar configuracion_asignacion local
...
export default {
  query: async (text, params) => {
    ...
    try {
      ...
      console.warn(`[DB] Error en base de datos real: ${errorMsg}. Cambiando a modo LOCAL.`);
      isLocalMode = true;
